...
//                });
    ...
    private ServerWebExchange userIdOrTempIdTransport(UserInfo info, ServerWebExchange exchange) {
        ...
        //用户登录了
        if(info != null){
            newReqbuilder.header(SysRedisConstant.USERID_HEADER,info.getId().toString());
        }
        //用户没登录
        String userTempId = getUserTempId(exchange);
        newReqbuilder.header(SysRedisConstant.USERTEMPID_HEADER,userTempId);
...
//            request.getHeaders().add();
    ...
    private UserInfo getTokenUserInfo(String tokenValue) {
        //登录了redis中会存在用户的token
        String json = redisTemplate.opsForValue().get(SysRedisConstant.USER_TOKEN + tokenValue);
